...
def normal_T_schedule(total_step, timesteps, curr_step):

    return timesteps

def unsmooth_T_schedule(total_step, timesteps, curr_step):
    """
    Args:
        curr_step: Current training steps
        total_step: Total training steps
        timesteps: Original T (total timesteps involved in training)

    Returns:
        modified_T: modified T
    """
    ...
    if curr_step <= modified_steps / 4:
        modified_T = int(timesteps / 5)

    elif curr_step <= 2 * modified_steps / 4:
        modified_T = int(2 * timesteps / 5)

    elif curr_step <= 3 * modified_steps / 4:
        modified_T = int(3 * timesteps / 5)

    else:
        modified_T = int(4 * timesteps / 5)

    return min(modified_T, timesteps)
...
class GaussianDiffusion(nn.Module):
    def __init__(
        ...
        *,
        ...
        # T schedule
        if T_schedule == 'normal':
            self.T_modifier = partial(normal_T_schedule,
                                      total_step=train_num_steps,
                                      timesteps=timesteps)

        elif T_schedule == "unsmooth":
            self.T_modifier = partial(unsmooth_T_schedule,
                                      total_step=train_num_steps,
                                      timesteps=timesteps)

        elif T_schedule == "linear":
            self.T_modifier = partial(linear_T_schedule,
                                      total_step=train_num_steps,
                                      timesteps=timesteps)

        elif T_schedule == "polynomial":
            self.T_modifier = partial(polynomial_T_schedule,
                                      total_step=train_num_steps,
                                      timesteps=timesteps,
                                      power=polynomial_T_power)

        elif T_schedule == "cosine":
            self.T_modifier = partial(cosine_T_schedule,
                                      total_step=train_num_steps,
                                      timesteps=timesteps)

        else:
            raise ValueError(f'unknown T schedule {T_schedule}')
    ...
    def forward(self, txt_latent, mask, class_id, curr_training_step=None, *args, **kwargs):
        ...
        if curr_training_step is None:
            T = self.num_timesteps
        else:
            T = self.T_modifier(curr_step=curr_training_step)
...
class Trainer(object):
    def __init__(
        ...
        *,
        ...
        print(f"Succeed in loading bart model: {args.enc_dec_model}")
        ...
        print(f"Succeed in loading tokenizer {args.enc_dec_model}")
        ...
        if optimizer_type == "lion":
            ...
            self.opt = optimizer.get_lion_optimizer(diffusion.parameters(), lr=train_lr, betas=adam_betas, weight_decay=adam_weight_decay)
        else:
        ...
        # make a directory for saving evaluation result if it doesn't exist
        self.evaluation_dir_path = f"{self.results_folder}/evaluation_results"
        print(f"evaluation folder: {self.evaluation_dir_path}")
        if not os.path.exists(self.evaluation_dir_path):
            os.makedirs(self.evaluation_dir_path)
    ...
    def sample(self, num_samples=None, class_id=None, seed=42, test=False):
        ...
        print(f"exists(class_id): {exists(class_id)}")
        ...
        for strategy, all_texts_list in text_generations.items():
            ...
            print(f"Perplexity: {strategy}, {class_id_prefix}", metrics[f"model/{strategy}/{class_id_prefix}perplexity"])

            # unique wordcount
            metrics[f"model/{strategy}/{class_id_prefix}unique_wordcount"] = evaluation.compute_wordcount(all_texts_list)
            print(f"wordcount: {strategy}, {class_id_prefix}", metrics[f"model/{strategy}/{class_id_prefix}unique_wordcount"])
        ...
        metric_file_path = os.path.join(self.evaluation_dir_path, f"metrics_{self.step}.txt")
        reference_file_path = os.path.join(self.evaluation_dir_path, f"reference_{self.step}.txt")
        with open(metric_file_path, 'w') as f:
            for k, v in metrics.items():
                f.write(f'{k}, {v}\n')
            print(f"Saving the metrics at {metric_file_path}")

        with open(reference_file_path, 'w') as f:
            for k, v in self.reference_dict.items():
                f.write(f'{k}, {v}\n')
            print(f"Saving the metrics at {reference_file_path}")
            ...
            while self.step < self.train_num_steps:
                ...
                for grad_accum_step in range(self.gradient_accumulate_every):
                    ...
                    with self.accelerator.autocast():
                        loss = self.diffusion(latent, mask, curr_training_step=self.step, class_id=(data['label'] if self.diffusion.diffusion_model.class_conditional else None))
                    ...
                    if self.step % 50 == 0:
                        ...
                        with torch.no_grad():
                            ...
                            for grad_accum_step in range(self.gradient_accumulate_every):
                                ...
                                mask = data['attention_mask'].bool()
                                with self.accelerator.autocast():
                                    # adding the current training step to model's forward function
                            ...
                            logs = {"loss": total_loss,
                                    ...
                                    "step": self.step,
                                    "T": self.diffusion.T_modifier(curr_step=self.step), # need to test
